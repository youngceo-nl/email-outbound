# Baseline measurements — 2026-08-06

Supporting data for [scoring-targets.md](scoring-targets.md). Read that first; this is
the workings, kept so any number can be re-derived rather than trusted.

Measured from the three real runs on record (2026-08-03 "C3", 2026-08-04 "C4",
2026-08-05), via `qualification_runs`, `qualification_run_leads`,
`lead_qualification_decisions` and `crawl_logs`. Queries at the bottom.
