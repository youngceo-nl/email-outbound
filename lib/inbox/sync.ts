...
import { classifyReplySentiment } from "@/lib/openai/classify-reply";
...
// Auth-agnostic core of app/actions/inbox.ts's syncInbox — extracted so the
// sync-inbox-scheduled Inngest job keeps inbox_messages.sentiment fresh on a
// timer, not just whenever someone clicks "Refresh" on Outreach Ready (which
// the warm-followup chain otherwise depends on to ever notice a reply).
//
